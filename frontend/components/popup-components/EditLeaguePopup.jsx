...
import { updateLeague } from "@/store/features/leagues/leagueSlice";

export default function EditLeaguePopup ({league_name , league_id}){
    const [leagueName , setLeagueName] = useState(league_name)
    ...
    const handleUpdateLeague = async ()=>{
       if (leagueName.trim().length >= 3 || leagueName.trim().length <= 30){
         try {
           await dispatch(updateLeague({id : league_id , name : leagueName})).unwrap()
